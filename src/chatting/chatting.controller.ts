...
export class ChattingController {
    ...
    @ApiCreatedResponse({ description: '등록된 친구중 친구정보를 변경합니다.'})
    async GetChattingList(@Param('id') id: number, @Query('cursor') cursor: number) : Promise<Chatting[]> {
        return await this.chattingService.getChattingList(id, cursor)
    }

    @Post('add')
    @ApiOperation({ summary: '채팅방방 생성하기 API', description: '채팅방을 생성합니다.' })
    @ApiCreatedResponse({ description: '참가자를 선택하면 자동으로 채팅방 종류가 만들어지고, 채팅방이 생성이 됩니다.'})
    ...
    @Post('invite')
    @ApiOperation({ summary: '채팅방에 초대하기 API', description: '채팅방에 원하는 참가자를 초대합니다.' })
    @ApiCreatedResponse({ description: '채팅방에 원하는 참가자를 초대합니다.'})
    async InviteRoom(@Body() inviteToRoom: InviteToRoom, @GetUser() user: User) : Promise<Participant[]> {
