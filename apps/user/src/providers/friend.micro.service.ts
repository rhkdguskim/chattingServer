...
export class FriendMicroService implements FriendService {
  ...
  getFriends(id: number): Promise<FriendEntity[]> {
    return lastValueFrom<FriendEntity[]>(
      this.tcpClientAdaptor.send<FriendEntity[]>({ cmd: FIND_ALL_FRIEND }, id)
    );
  }
  ...
  addFriend(createFriend: CreateFriendRequest): Promise<CreateFriendResponse> {
    return lastValueFrom<CreateFriendResponse>(
      this.tcpClientAdaptor.send<CreateFriendResponse>(
        { cmd: ADD_FRIEND },
        createFriend
      )
    );
  ...
  delFriend(delFriend: DeleteFriendRequest): Promise<any> {
    return lastValueFrom<FriendEntity>(
      this.tcpClientAdaptor.send<FriendEntity>(
        { cmd: DELETE_FRIEND },
        delFriend
      )
    );
